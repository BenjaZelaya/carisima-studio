...
const Pago = () => {
  ...
  // Datos de la nueva reserva (si hay)
  ...
  // Estados para turnos pendientes
  const [turnoPendiente, setTurnoPendiente] = useState(null);
  const [cargandoTurnos, setCargandoTurnos] = useState(true);

  // Si hay un turno pendiente, usar sus datos
  if (turnoPendiente) {
  ...
  // ─── Verificar turnos pendientes al cargar ──────────────────────────────────
  ...
  useEffect(() => {
    const cargarTurnosPendientes = async () => {
      ...
      // Si vienen servicios nuevos desde location.state, NO mostrar modal de turno pendiente
      ...
      try {
        ...
        if (res.ok && data.data && Array.isArray(data.data)) {
          // Buscar turno en estado "pendiente"
  ...
  useEffect(() => {
    // Esperar a que termine de cargar los turnos
    if (cargandoTurnos) return;

    // Si no hay servicios ni turno pendiente, ir a reservar
    if (servicios.length === 0 && !turnoPendiente) {
      navigate("/reservar");
    }
  }, [cargandoTurnos, servicios.length, turnoPendiente, navigate]);

  // ─── Continuar con reserva pendiente ───────────────────────────────────────
